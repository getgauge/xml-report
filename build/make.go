...
func createDistro() {
	packageName := fmt.Sprintf("%s-%s-%s.%s", xmlReport, getPluginVersionWithBuildInfo(), getGOOS(), getArch())
...
func compileGoPackage(packageName string) {
	runProcess("go", "build", "-o", getGaugeExecutablePath(xmlReport))
}
...
func copyPluginFiles(destDir string) {
	...
	if getGOOS() == "windows" {
		files[filepath.Join(getBinDir(), xmlReport+".exe")] = bin
	} else {
...
func installPlugin(installPrefix string) {
	...
	pluginInstallPath := filepath.Join(installPrefix, xmlReport, getPluginVersionWithBuildInfo())
